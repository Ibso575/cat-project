...
const Navbar = () => {
  ...
  const changeLanguage = (lng) => {
    ...
    i18n.changeLanguage(lng); // i18next tilini ham o'zgartirish
    setIsLangOpen(false);
  };
  ...
  return (
    <header className="w-full font-sans text-xs antialiased relative">
          ...
          <nav className="flex-1">
            <ul className="flex items-center justify-center gap-6">
              {topNavItems.map((item) => (
                <li key={item.key} className={`cursor-pointer hover:text-[#ff9800] transition-colors ${item.active ? 'text-[#ff9800] font-bold' : ''}`}>
                  {t(item.key)}
                </li>
              ))}
            </ul>
      ...
      <div className="bg-white dark:bg-slate-900 py-3 lg:py-4 shadow-sm transition-colors duration-300 relative z-40">
        <div className="container mx-auto flex items-center justify-between px-4">
          ...
          <form onSubmit={handleSearch} className="hidden lg:flex flex-1 mx-10 relative max-w-2xl">
            ...
            />
            <button type="submit" className="absolute right-4 top-1/2 -translate-y-1/2 text-[#4d4d4d] dark:text-gray-500 hover:text-[#ff9800] transition-colors"><Search size={20} /></button>
          </form>
          ...
          <div className="flex items-center gap-4">
            <div className="hidden lg:flex items-center gap-4">
                          ...
                          {cartItems.map((item) => (
                            <li key={item.id} className="flex items-center gap-3 px-5 py-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <Link to={`/products/${item.id}`} onClick={() => setIsCartOpen(false)} className="text-sm font-medium text-[#333] dark:text-gray-200 hover:text-[#ff9800] line-clamp-2">
                                  {item.name}
                                </Link>
                                <p className="text-xs text-gray-400 mt-0.5">{item.price} ₽</p>
                              </div>
                              <div className="flex items-center gap-1.5">
                                <button onClick={() => dispatch(decreaseQuantity(item.id))} className="w-7 h-7 rounded-full border dark:border-slate-600 flex items-center justify-center hover:border-[#ff9800] transition">−</button>
                                <span className="text-sm font-bold w-5 text-center dark:text-white">{item.quantity}</span>
                                <button onClick={() => dispatch(addToCart(item))} className="w-7 h-7 rounded-full border dark:border-slate-600 flex items-center justify-center hover:border-[#ff9800] transition">+</button>
                              </div>
                              <div className="flex flex-col items-end gap-1">
                                <span className="text-sm font-bold dark:text-white">{(item.price * item.quantity).toLocaleString()} ₽</span>
                                <button onClick={() => dispatch(removeFromCart(item.id))} className="text-gray-300 hover:text-red-400"><Trash2 size={14} /></button>
                              </div>
                            </li>
                          ))}
                        </ul>
                        <div className="px-5 py-4 bg-gray-50 dark:bg-slate-900 border-t dark:border-slate-700">
                          <div className="flex justify-between items-center mb-3">
                            <span className="text-sm text-gray-500">{t('total')}:</span>
                            <span className="text-lg font-black text-[#ff9800]">{cartTotal.toLocaleString()} ₽</span>
                          </div>
                          <Link to="/checkout/cart" onClick={() => setIsCartOpen(false)} className="block w-full text-center bg-[#ff9800] text-white font-semibold py-3 rounded-full text-sm hover:bg-[#e68600] transition-all">
                            {t('checkout')} →
                          </Link>
                        </div>
                      </>
              ...
              {/* THEME TOGGLE */}
              <button onClick={() => dispatch(toggleTheme())} className="w-10 h-10 border border-gray-200 dark:border-slate-700 rounded-full flex items-center justify-center text-gray-400 hover:text-[#ff9800] transition-all">
                {theme === 'light' ? <Moon size={20} /> : <Sun size={20} />}
              </button>

              {/* LANG SWITCHER */}
              <div className="relative" ref={langRef}>
                <button onClick={() => setIsLangOpen(!isLangOpen)} className="px-3 h-10 border border-gray-200 dark:border-slate-700 rounded-full flex items-center gap-2 text-gray-600 dark:text-gray-300 hover:bg-gray-50 transition-all">
                  <Globe size={18} className="text-[#bfbfbf]" />
                  <span className="font-bold text-xs">{currentLanguage.toUpperCase()}</span>
                  <ChevronDown size={14} className={`transition-transform ${isLangOpen ? 'rotate-180' : ''}`} />
                </button>
                {isLangOpen && (
                  <div className="absolute right-0 top-[calc(100%+8px)] w-24 bg-white dark:bg-slate-800 rounded-xl shadow-xl border dark:border-slate-700 z-[200] overflow-hidden">
                    {languages.map((lng) => (
                      <button key={lng.code} onClick={() => changeLanguage(lng.code)} className={`w-full px-4 py-2 text-left text-xs font-bold hover:bg-gray-50 dark:hover:bg-slate-700 ${currentLanguage === lng.code ? 'text-[#ff9800]' : 'dark:text-gray-300'}`}>
                        {lng.name}
                      </button>
                    ))}
                  </div>
            ...
            <button className="lg:hidden p-2 text-gray-600 dark:text-gray-400" onClick={() => setIsMenuOpen(true)}>
              <Menu size={30} />
            </button>
          </div>
        </div>
      ...
      {/* 3. CATEGORIES BAR */}
      <div className="hidden lg:block bg-white dark:bg-slate-900 pb-3 border-b border-gray-100 dark:border-slate-800 relative z-30">
        <div className="container mx-auto px-4">
          <ul className="flex items-center justify-between text-[#4d4d4d] dark:text-gray-300 whitespace-nowrap">
            {categoryItems.map((item, index) => (
              <li key={index} className="cursor-pointer py-2 px-1 hover:text-[#ff9800] transition-colors flex items-center gap-1.5 uppercase tracking-wide font-bold text-[11px]">
                {item.label}
      ...
      {/* 4. MOBILE DRAWER */}
      <div className={`lg:hidden fixed inset-0 bg-white dark:bg-slate-950 z-[100] transition-transform duration-300 ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-4 flex flex-col h-full">
          <div className="flex justify-between items-center mb-8 border-b dark:border-slate-800 pb-4">
            <img src={logo} alt="Logo" className="h-9" />
            <button onClick={() => setIsMenuOpen(false)} className="p-2 bg-gray-100 dark:bg-slate-800 rounded-full">
              <X size={24} className="dark:text-gray-300" />
            </button>
          </div>
          <div className="flex-1 overflow-y-auto space-y-6">
            <form onSubmit={handleSearch} className="relative">
              <input type="search" placeholder="Поиск товаров" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-gray-100 dark:bg-slate-900 rounded-xl p-4 pr-12 outline-none dark:text-gray-200" />
              <button type="submit" className="absolute right-4 top-4 text-gray-400"><Search size={20} /></button>
            </form>
            <nav>
              <h3 className="font-bold text-lg mb-4 text-[#316c8c] border-l-4 border-[#316c8c] pl-3">Категории</h3>
              <ul className="space-y-4">
                {categoryItems.map((item, index) => (
                  <li key={index} className="flex items-center justify-between text-lg text-gray-700 dark:text-gray-300 border-b dark:border-slate-800 pb-2">
                    {item.label}
