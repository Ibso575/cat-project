          ...
          <ul className="flex items-center justify-between text-[#4d4d4d] dark:text-gray-300 whitespace-nowrap">
            {categoryItems.map((item, index) => (
              <li key={index} className="flex items-center gap-1.5">
                <Link
                  to={item.key === 'promotions' ? '/sales' : `/catalog?category=${encodeURIComponent(item.key)}`}
                ...
                {categoryItems.map((item, index) => (
                  <li key={index}>
                    <Link
                      to={item.key === 'promotions' ? '/sales' : `/catalog?category=${encodeURIComponent(item.key)}`}
                      onClick={() => setIsMenuOpen(false)}
                      className="flex items-center justify-between text-lg text-gray-700 dark:text-gray-300 border-b border-gray-50 dark:border-slate-800 pb-2 active:text-[#ff9800] dark:active:text-[#ff9800] transition-colors w-full"
                    >
                      {item.label}
